--  This spec has been automatically generated from STM32F105xx.svd

pragma Restrictions (No_Elaboration_Code);
pragma Ada_2012;

with System;

package STM32.EXTI is
   pragma Preelaborate;

   ---------------
   -- Registers --
   ---------------

   ------------------
   -- IMR_Register --
   ------------------

   ------------
   -- IMR.MR --
   ------------

   --  IMR_MR array element
   subtype IMR_MR_Element is STM32.Bit;

   --  IMR_MR array
   type IMR_MR_Field_Array is array (0 .. 19) of IMR_MR_Element
     with Component_Size => 1, Size => 20;

   --  Type definition for IMR_MR
   type IMR_MR_Field
     (As_Array : Boolean := False)
   is record
      case As_Array is
         when False =>
            --  MR as a value
            Val : STM32.UInt20;
         when True =>
            --  MR as an array
            Arr : IMR_MR_Field_Array;
      end case;
   end record
     with Unchecked_Union, Size => 20;

   for IMR_MR_Field use record
      Val at 0 range 0 .. 19;
      Arr at 0 range 0 .. 19;
   end record;

   --  Interrupt mask register (EXTI_IMR)
   type IMR_Register is record
      --  Interrupt Mask on line 0
      MR             : IMR_MR_Field := (As_Array => False, Val => 16#0#);
      --  unspecified
      Reserved_20_31 : STM32.UInt12 := 16#0#;
   end record
     with Volatile_Full_Access, Size => 32,
          Bit_Order => System.Low_Order_First;

   for IMR_Register use record
      MR             at 0 range 0 .. 19;
      Reserved_20_31 at 0 range 20 .. 31;
   end record;

   ------------------
   -- EMR_Register --
   ------------------

   ------------
   -- EMR.MR --
   ------------

   --  EMR_MR array element
   subtype EMR_MR_Element is STM32.Bit;

   --  EMR_MR array
   type EMR_MR_Field_Array is array (0 .. 19) of EMR_MR_Element
     with Component_Size => 1, Size => 20;

   --  Type definition for EMR_MR
   type EMR_MR_Field
     (As_Array : Boolean := False)
   is record
      case As_Array is
         when False =>
            --  MR as a value
            Val : STM32.UInt20;
         when True =>
            --  MR as an array
            Arr : EMR_MR_Field_Array;
      end case;
   end record
     with Unchecked_Union, Size => 20;

   for EMR_MR_Field use record
      Val at 0 range 0 .. 19;
      Arr at 0 range 0 .. 19;
   end record;

   --  Event mask register (EXTI_EMR)
   type EMR_Register is record
      --  Event Mask on line 0
      MR             : EMR_MR_Field := (As_Array => False, Val => 16#0#);
      --  unspecified
      Reserved_20_31 : STM32.UInt12 := 16#0#;
   end record
     with Volatile_Full_Access, Size => 32,
          Bit_Order => System.Low_Order_First;

   for EMR_Register use record
      MR             at 0 range 0 .. 19;
      Reserved_20_31 at 0 range 20 .. 31;
   end record;

   -------------------
   -- RTSR_Register --
   -------------------

   -------------
   -- RTSR.TR --
   -------------

   --  RTSR_TR array element
   subtype RTSR_TR_Element is STM32.Bit;

   --  RTSR_TR array
   type RTSR_TR_Field_Array is array (0 .. 19) of RTSR_TR_Element
     with Component_Size => 1, Size => 20;

   --  Type definition for RTSR_TR
   type RTSR_TR_Field
     (As_Array : Boolean := False)
   is record
      case As_Array is
         when False =>
            --  TR as a value
            Val : STM32.UInt20;
         when True =>
            --  TR as an array
            Arr : RTSR_TR_Field_Array;
      end case;
   end record
     with Unchecked_Union, Size => 20;

   for RTSR_TR_Field use record
      Val at 0 range 0 .. 19;
      Arr at 0 range 0 .. 19;
   end record;

   --  Rising Trigger selection register (EXTI_RTSR)
   type RTSR_Register is record
      --  Rising trigger event configuration of line 0
      TR             : RTSR_TR_Field := (As_Array => False, Val => 16#0#);
      --  unspecified
      Reserved_20_31 : STM32.UInt12 := 16#0#;
   end record
     with Volatile_Full_Access, Size => 32,
          Bit_Order => System.Low_Order_First;

   for RTSR_Register use record
      TR             at 0 range 0 .. 19;
      Reserved_20_31 at 0 range 20 .. 31;
   end record;

   -------------------
   -- FTSR_Register --
   -------------------

   -------------
   -- FTSR.TR --
   -------------

   --  FTSR_TR array element
   subtype FTSR_TR_Element is STM32.Bit;

   --  FTSR_TR array
   type FTSR_TR_Field_Array is array (0 .. 19) of FTSR_TR_Element
     with Component_Size => 1, Size => 20;

   --  Type definition for FTSR_TR
   type FTSR_TR_Field
     (As_Array : Boolean := False)
   is record
      case As_Array is
         when False =>
            --  TR as a value
            Val : STM32.UInt20;
         when True =>
            --  TR as an array
            Arr : FTSR_TR_Field_Array;
      end case;
   end record
     with Unchecked_Union, Size => 20;

   for FTSR_TR_Field use record
      Val at 0 range 0 .. 19;
      Arr at 0 range 0 .. 19;
   end record;

   --  Falling Trigger selection register (EXTI_FTSR)
   type FTSR_Register is record
      --  Falling trigger event configuration of line 0
      TR             : FTSR_TR_Field := (As_Array => False, Val => 16#0#);
      --  unspecified
      Reserved_20_31 : STM32.UInt12 := 16#0#;
   end record
     with Volatile_Full_Access, Size => 32,
          Bit_Order => System.Low_Order_First;

   for FTSR_Register use record
      TR             at 0 range 0 .. 19;
      Reserved_20_31 at 0 range 20 .. 31;
   end record;

   --------------------
   -- SWIER_Register --
   --------------------

   -----------------
   -- SWIER.SWIER --
   -----------------

   --  SWIER array element
   subtype SWIER_Element is STM32.Bit;

   --  SWIER array
   type SWIER_Field_Array is array (0 .. 19) of SWIER_Element
     with Component_Size => 1, Size => 20;

   --  Type definition for SWIER
   type SWIER_Field
     (As_Array : Boolean := False)
   is record
      case As_Array is
         when False =>
            --  SWIER as a value
            Val : STM32.UInt20;
         when True =>
            --  SWIER as an array
            Arr : SWIER_Field_Array;
      end case;
   end record
     with Unchecked_Union, Size => 20;

   for SWIER_Field use record
      Val at 0 range 0 .. 19;
      Arr at 0 range 0 .. 19;
   end record;

   --  Software interrupt event register (EXTI_SWIER)
   type SWIER_Register is record
      --  Software Interrupt on line 0
      SWIER          : SWIER_Field := (As_Array => False, Val => 16#0#);
      --  unspecified
      Reserved_20_31 : STM32.UInt12 := 16#0#;
   end record
     with Volatile_Full_Access, Size => 32,
          Bit_Order => System.Low_Order_First;

   for SWIER_Register use record
      SWIER          at 0 range 0 .. 19;
      Reserved_20_31 at 0 range 20 .. 31;
   end record;

   -----------------
   -- PR_Register --
   -----------------

   -----------
   -- PR.PR --
   -----------

   --  PR array element
   subtype PR_Element is STM32.Bit;

   --  PR array
   type PR_Field_Array is array (0 .. 19) of PR_Element
     with Component_Size => 1, Size => 20;

   --  Type definition for PR
   type PR_Field
     (As_Array : Boolean := False)
   is record
      case As_Array is
         when False =>
            --  PR as a value
            Val : STM32.UInt20;
         when True =>
            --  PR as an array
            Arr : PR_Field_Array;
      end case;
   end record
     with Unchecked_Union, Size => 20;

   for PR_Field use record
      Val at 0 range 0 .. 19;
      Arr at 0 range 0 .. 19;
   end record;

   --  Pending register (EXTI_PR)
   type PR_Register is record
      --  Pending bit 0
      PR             : PR_Field := (As_Array => False, Val => 16#0#);
      --  unspecified
      Reserved_20_31 : STM32.UInt12 := 16#0#;
   end record
     with Volatile_Full_Access, Size => 32,
          Bit_Order => System.Low_Order_First;

   for PR_Register use record
      PR             at 0 range 0 .. 19;
      Reserved_20_31 at 0 range 20 .. 31;
   end record;

   -----------------
   -- Peripherals --
   -----------------

   --  EXTI
   type EXTI_Peripheral is record
      --  Interrupt mask register (EXTI_IMR)
      IMR   : IMR_Register;
      --  Event mask register (EXTI_EMR)
      EMR   : EMR_Register;
      --  Rising Trigger selection register (EXTI_RTSR)
      RTSR  : RTSR_Register;
      --  Falling Trigger selection register (EXTI_FTSR)
      FTSR  : FTSR_Register;
      --  Software interrupt event register (EXTI_SWIER)
      SWIER : SWIER_Register;
      --  Pending register (EXTI_PR)
      PR    : PR_Register;
   end record
     with Volatile;

   for EXTI_Peripheral use record
      IMR   at 0 range 0 .. 31;
      EMR   at 4 range 0 .. 31;
      RTSR  at 8 range 0 .. 31;
      FTSR  at 12 range 0 .. 31;
      SWIER at 16 range 0 .. 31;
      PR    at 20 range 0 .. 31;
   end record;

   --  EXTI
   EXTI_Periph : aliased EXTI_Peripheral
     with Import, Address => EXTI_Base;

end STM32.EXTI;
